...
class Viz_Attn_OCT_Model(nn.Module):
    def __init__(self, input_size):
        super(Viz_Attn_OCT_Model, self).__init__()
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
        ...
        print(f'Using attention in OCT model')
        att_input_size = int(
            input_size[0]/2) * int(input_size[1]/2) * int(input_size[2]/2)
        self.trans_block1 = EPABlock(
            input_size=att_input_size, hidden_size=32, proj_size=64, num_heads=4, dropout_rate=0.15, pos_embed=False, return_attn=True)
        self.max_pool1 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))

        att_input_size = int(
            input_size[0]/4) * int(input_size[1]/4) * int(input_size[2]/4)
        self.trans_block2 = EPABlock(
            input_size=att_input_size, hidden_size=32, proj_size=64, num_heads=4, dropout_rate=0.15, pos_embed=False)
        self.max_pool2 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))
...
class Viz_Cross_Attn_OCT_Model(nn.Module):
    def __init__(self, input_size, att_type):
        super(Viz_Cross_Attn_OCT_Model, self).__init__()
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
        ...
        print(f'Using {att_type} attention in OCT model')
        if att_type == 'CrossSIIS':
            self.trans_block1 = CrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15, return_attn=True)
            self.trans_block2 = CrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15, return_attn=True)
        elif att_type == 'CrossSIMN':
            self.trans_block1 = CrossAttentionBlockSIMN(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15, return_attn=True)
            self.trans_block2 = CrossAttentionBlockSIMN(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15, return_attn=True)
        elif att_type == 'SplitCrossSIIS':
            self.trans_block1 = SplitCrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                             dropout_rate=0.15, return_attn=True)
            self.trans_block2 = SplitCrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                             dropout_rate=0.15, return_attn=True)
        self.max_pool1 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))
        self.max_pool2 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))
    ...
    def forward(self, x):
        ...
        x = self.conv_block2(x)
        # x, attn = self.trans_block1(x)
        x, attn_1, attn_2 = self.trans_block1(x)
        x = self.max_pool1(x)
        # print('3', x.shape)
        x = self.conv_block3(x)
        # print('4', x.shape)
        x = self.conv_block4(x)
        x, _, _ = self.trans_block2(x)
        x = self.max_pool2(x)
        # print('5', x.shape)
        x = self.conv_block5(x)
        # print('6', x.shape)
...
class Attn_OCT_Model(nn.Module):
    def __init__(self, input_size):
        super(Attn_OCT_Model, self).__init__()
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
...
class Viz_Cross_Attn_OCT_Model_After_Max(nn.Module):
    def __init__(self, input_size, att_type):
        super(Viz_Cross_Attn_OCT_Model_After_Max, self).__init__()
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
        ...
        if att_type == 'CrossSIIS':
            ...
            self.trans_block1 = MultiSplitCrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                                  dropout_rate=0.15)
            self.trans_block2 = MultiSplitCrossAttentionBlockSIIS(hidden_size=32, num_heads=4,
                                                                  dropout_rate=0.15)
        elif att_type == 'CrossSIMN':
            self.trans_block1 = CrossAttentionBlockSIMN(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15)
            ...
            self.trans_block1 = CrossAttentionBlockMNNM(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15)
            self.trans_block2 = CrossAttentionBlockMNNM(hidden_size=32, num_heads=4,
                                                        dropout_rate=0.15)
        elif att_type == 'EPA':
            att_input_size = int(
                input_size[0]/2) * int(input_size[1]/2) * int(input_size[2]/2)
            self.trans_block1 = EPABlock(input_size=att_input_size, hidden_size=32,
                                         proj_size=64, num_heads=4, dropout_rate=0.15, pos_embed=False)

            att_input_size = int(
                input_size[0]/4) * int(input_size[1]/4) * int(input_size[2]/4)
            self.trans_block2 = EPABlock(input_size=att_input_size, hidden_size=32,
                                         proj_size=64, num_heads=4, dropout_rate=0.15, pos_embed=False)
        self.max_pool1 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))
        self.max_pool2 = torch.nn.MaxPool3d(kernel_size=(2, 2, 2))
...
class GradCAM_Cross_Attn_OCT_Model(nn.Module):
    def __init__(self, input_size, att_type):
        super(GradCAM_Cross_Attn_OCT_Model, self).__init__()
        # Original layers and blocks initialization
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
    ...
    def register_hooks(self):
        """Register hooks to the nn.Conv3d layer inside conv_block1."""
        def forward_hook(module, input, output):
            # print(f'hooked_activations in shape: {input[0].shape} hooked_activations out shape: {output.shape}')
            self.hooked_activations = output

        def backward_hook(module, grad_in, grad_out):
            # print(f'grad in shape: {grad_in[1].shape} grad out shape: {grad_out[0].shape}')
            self.gradients = grad_out[0]

        # Access the nn.Conv3d layer in conv_block1
        conv3d_layer = self.conv_block1[0]  # First layer of conv_block1
    ...
    def compute_gradcam_batch(self):
        ...
        for i in range(batch_size):
            print(
                f'self.gradients[i] {self.gradients[i].shape} max/min: {torch.max(self.gradients[i])}/{torch.min(self.gradients[i])}')
            ...
            print(f'hm 1 max/min: {torch.max(heatmap)}/{torch.min(heatmap)}')
            heatmap = torch.nn.functional.relu(heatmap)
            print(f'hm 2 max/min: {torch.max(heatmap)}/{torch.min(heatmap)}')
            heatmap = torch.nn.functional.normalize(heatmap)
            # heatmap = heatmap / torch.max(heatmap)
            print(f'hm 3 max/min: {torch.max(heatmap)}/{torch.min(heatmap)}')
...
class Cross_Attn_OCT_Model(nn.Module):
    def __init__(self, input_size, att_type):
        super(Cross_Attn_OCT_Model, self).__init__()
        self.conv_block1 = make_block(
            in_channels=1, out_channels=32, kernel_size=7, stride=2, padding=3, num_features=32)
        self.conv_block2 = make_block(
            in_channels=32, out_channels=32, kernel_size=5, stride=1, padding='same', num_features=32)
